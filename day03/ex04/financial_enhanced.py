...
def page_parser(page, ar1, ar2):
    ...
    for val in value:
        if (val.span.text == ar2):
            ...
            nums1 = val.find_all('div', class_='Ta(c) Py(6px) Bxz(bb) BdB Bdc($seperatorColor) Miw(120px) Miw('
                                               '100px)--pnclg Bgc($lv1BgColor) fi-row:h_Bgc($hoverBgColor) D(tbc)')
            nums2 = val.find_all('div', class_='Ta(c) Py(6px) Bxz(bb) BdB Bdc($seperatorColor) Miw(120px) Miw('
                                               '100px)--pnclg D(tbc)')
